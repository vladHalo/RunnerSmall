...
namespace Core.Scripts.Views
{
    ...
    {
        private readonly string[] Suffixes = { "-Image", "-Color", "-Text", "-Pos" };
        ...
        public void ChangeButton(int index)
        {
            if (_buttonsList[index].image != null && _buttonsList[index].sprites.Count >= 2)
            {
                ...
                _buttonsList[index].image.rectTransform.anchoredPosition =
                    new Vector2(_buttonsList[index].position[number],
                        _buttonsList[index].image.rectTransform.anchoredPosition.y);
            }
        ...
        private void StartChangeButton()
        {
            foreach (var t in _buttonsList)
            {
                ...
                if (ES3.KeyExists(t.nameSave + Suffixes[0]) && t.image != null)
                {
                    ...
                    t.image.rectTransform.anchoredPosition =
                        new Vector2(t.position[index], t.image.rectTransform.anchoredPosition.y);
                }
